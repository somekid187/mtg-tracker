...
BEGIN
    DECLARE EXIT HANDLER FOR SQLEXCEPTION
        BEGIN
            SET out_response = JSON_OBJECT('success', FALSE, 'message', 'An error occurred while fetching the team.');
        END;

    IF (in_pk_team IS NULL) THEN
        SET out_response = JSON_OBJECT('success', FALSE, 'message', 'Team ID cannot be null.');
        LEAVE proc;
    END IF;
