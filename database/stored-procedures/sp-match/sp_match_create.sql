...
proc:BEGIN
    DECLARE EXIT HANDLER FOR SQLEXCEPTION
        BEGIN
            SET out_response = JSON_OBJECT('success', FALSE, 'message', 'An error occurred while creating the match.');
        END;
    IF in_format IS NULL OR in_format = '' THEN
        SET out_response = JSON_OBJECT('success', FALSE, 'message', 'Match format cannot be empty.');
        LEAVE proc;
    END IF;

    IF in_startingLife IS NULL OR in_startingLife <= 0 THEN
        SET out_response = JSON_OBJECT('success', FALSE, 'message', 'Starting life must be a positive integer.');
        LEAVE proc;
    END IF;

    IF in_startTime IS NULL THEN
        SET out_response = JSON_OBJECT('success', FALSE, 'message', 'Start time cannot be null.');
        LEAVE proc;
    END IF;

    IF in_isTeamMatch IS NULL THEN
        SET out_response = JSON_OBJECT('success', FALSE, 'message', 'isTeamMatch cannot be null.');
        LEAVE proc;
    END IF;

    IF in_fk_appUser_creates IS NULL THEN
        SET out_response = JSON_OBJECT('success', FALSE, 'message', 'Creator user ID cannot be null.');
        LEAVE proc;
    END IF;

    IF NOT EXISTS (SELECT 1 FROM AppUser WHERE pk_appUser = in_fk_appUser_creates) THEN
        SET out_response = JSON_OBJECT('success', FALSE, 'message', 'Creator user not found.');
        LEAVE proc;
    END IF;
    ...
    SET out_response =
            JSON_OBJECT('success', TRUE, 'message', 'Match created successfully.', 'matchId', LAST_INSERT_ID());
END $$
